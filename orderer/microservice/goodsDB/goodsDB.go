...
import (
	...
	Goods "project1_update/orderer/microservice/services/GoodsDBrpc"
...
func PricingAndUpdateGoodsQuantity(code []int, amount []int) (total int64, err error) {
	...
	if err != nil {
		log.Fatalf("orderer conn to goodsDB err %v", err)
		return 0, err
	}
	defer conn.Close()
	...
	for i := 0; i <= len(code); i++ {
		...
		if err != nil {
			log.Fatalf("can't unmarshall in orderer microservice %v", err)
			return 0, err
		}
